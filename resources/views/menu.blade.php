@extends('layouts.app')

@section('content')
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <h2 class="text-2xl md:text-3xl font-bold tracking-tight text-gray-950">Menu</h2>
    <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">

        {{-- Tab Produk --}}
        <button onclick="switchTab('produk')" id="btn-produk"
            class="flex-1 sm:flex-none flex items-center justify-center gap-1 font-medium px-4 py-2 rounded-xl text-sm transition bg-[#c5cb9f] text-[#5a4a2f]">
            Produk 
        </button>

        {{-- Tab Kategori --}}
        <button onclick="switchTab('kategori')" id="btn-kategori"
            class="flex-1 sm:flex-none flex items-center justify-center gap-1 font-medium px-4 py-2 rounded-xl text-sm transition border border-gray-300 text-gray-700 hover:bg-gray-50">
            Kategori 
        </button>

        {{-- Tanggal --}}
        <div class="w-full sm:w-auto flex items-center justify-center gap-1 border border-gray-300 text-gray-700 font-medium px-4 py-2 rounded-xl text-sm">
            <span>{{ \Carbon\Carbon::now()->format('d/m/Y') }}</span>
        </div>
    </div>
</div>

{{-- ===================== TAB PRODUK ===================== --}}
<div id="tab-produk">
    {{-- Filter & Search: Susunan fleksibel agar tidak menabrak --}}
    <div class="flex flex-col md:flex-row flex-wrap gap-3 items-stretch md:items-center mb-6">
        
        <a href="{{ route('produk.create') }}"
            class="inline-flex items-center justify-center gap-2 bg-[#c5cb9f] text-[#5a4a2f] font-medium px-5 py-2.5 rounded-xl text-sm hover:bg-[#b5bb8f] transition w-full md:w-auto">
            <span class="material-icons-outlined !text-[18px]">add</span>
            Tambah Produk
        </a>

        <div class="flex gap-3 w-full md:w-auto">
            {{-- Filter Kategori --}}
            <div class="relative flex-1 md:flex-none">
                <button onclick="toggleDropdown('filter-kategori')"
                    class="w-full flex items-center justify-between gap-1 border border-gray-300 text-gray-600 px-4 py-2.5 rounded-xl text-sm hover:bg-gray-50 transition">
                    <span class="truncate">{{ request('kategori') ? \App\Models\Kategori::find(request('kategori'))->nama_kategori : 'Semua Kategori' }}</span>
                    <span class="material-icons-outlined !text-[16px] flex-shrink-0">expand_more</span>
                </button>
                <div id="filter-kategori" class="hidden absolute left-0 mt-2 w-48 bg-white rounded-xl shadow-lg z-20 border border-gray-100">
                    <a href="{{ route('menu', ['tab' => 'produk']) }}"
                        class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-t-xl">Semua Kategori</a>
                    @foreach(\App\Models\Kategori::all() as $kat)
                    <a href="{{ route('menu', ['tab' => 'produk', 'kategori' => $kat->id_kategori]) }}"
                        class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 last:rounded-b-xl">
                        {{ $kat->nama_kategori }}
                    </a>
                    @endforeach
                </div>
            </div>

            {{-- Filter Status --}}
            <div class="relative flex-1 md:flex-none">
                <button onclick="toggleDropdown('filter-status')"
                    class="w-full flex items-center justify-between gap-1 border border-gray-300 text-gray-600 px-4 py-2.5 rounded-xl text-sm hover:bg-gray-50 transition">
                    <span class="truncate">{{ request('status') ? ucfirst(request('status')) : 'Semua Status' }}</span>
                    <span class="material-icons-outlined !text-[16px] flex-shrink-0">expand_more</span>
                </button>
                <div id="filter-status" class="hidden absolute right-0 md:left-0 mt-2 w-40 bg-white rounded-xl shadow-lg z-20 border border-gray-100">
                    <a href="{{ route('menu', array_merge(request()->except('status'), ['tab' => 'produk'])) }}"
                        class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-t-xl">Semua Status</a>
                    <a href="{{ route('menu', array_merge(request()->all(), ['tab' => 'produk', 'status' => 'tersedia'])) }}"
                        class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">Tersedia</a>
                    <a href="{{ route('menu', array_merge(request()->all(), ['tab' => 'produk', 'status' => 'habis'])) }}"
                        class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-b-xl">Habis</a>
                </div>
            </div>
        </div>

        {{-- Search Bar --}}
        <form method="GET" action="{{ route('menu') }}" class="w-full md:flex-1">
            <input type="hidden" name="tab" value="produk">
            @if(request('kategori'))<input type="hidden" name="kategori" value="{{ request('kategori') }}">@endif
            @if(request('status'))<input type="hidden" name="status" value="{{ request('status') }}">@endif
            <div class="relative">
                <span class="material-icons-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 !text-[20px]">search</span>
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari nama menu"
                    class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#c5cb9f]">
            </div>
        </form>
    </div>

    {{-- Tabel Produk Kontainer Responsif --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm min-w-[800px] md:min-w-0">
                <thead>
                    <tr class="bg-[#8a7d55] text-white text-left">
                        <th class="px-6 py-4 font-medium w-16 text-center">#</th>
                        <th class="px-6 py-4 font-medium">Nama Menu</th>
                        <th class="px-6 py-4 font-medium">Kategori</th>
                        <th class="px-6 py-4 font-medium">Harga</th>
                        <th class="px-6 py-4 font-medium">Status</th>
                        <th class="px-6 py-4 font-medium text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @forelse($semua_produk as $i => $produk)
                    <tr class="hover:bg-gray-50/60 transition">
                        <td class="px-6 py-4 text-gray-400 text-center">{{ $i + 1 }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                @if($produk->gambar)
                                    <img src="{{ asset('storage/' . $produk->gambar) }}"
                                        alt="{{ $produk->nama_produk }}"
                                        class="w-10 h-10 rounded-lg object-cover flex-shrink-0">
                                @else
                                    <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-lg flex-shrink-0">🍽️</div>
                                @endif
                                <span class="font-medium text-gray-800">{{ $produk->nama_produk }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                @php
                                    $emojis = ['🍲','🥤','🍟','🍜','🍛','🧆'];
                                    echo $emojis[($produk->id_kategori - 1) % count($emojis)];
                                @endphp
                                <span class="text-gray-600 ml-1">{{ $produk->kategori->nama_kategori ?? '-' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-700 font-medium">
                            Rp {{ number_format($produk->harga, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4">
                            <button
                                onclick="toggleStatus(this, {{ $produk->id_produk }})"
                                data-status="{{ $produk->status }}"
                                class="text-xs font-semibold px-3 py-1 rounded-full border transition-all cursor-pointer focus:outline-none
                                    {{ ($produk->status ?? 'tersedia') === 'tersedia'
                                        ? 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100'
                                        : 'bg-red-50 text-red-500 border-red-200 hover:bg-red-100' }}">
                                {{ ($produk->status ?? 'tersedia') === 'tersedia' ? 'Tersedia' : 'Habis' }}
                            </button>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('produk.edit', $produk->id_produk) }}"
                                    class="p-1.5 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                    <span class="material-icons-outlined !text-[16px] text-gray-600 block">edit</span>
                                </a>
                                <form action="{{ route('produk.destroy', $produk->id_produk) }}" method="POST"
                                    onsubmit="return confirm('Hapus produk ini?')" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="p-1.5 border border-red-200 rounded-lg hover:bg-red-50 transition cursor-pointer">
                                        <span class="material-icons-outlined !text-[16px] text-red-400 block">delete</span>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                            Belum ada produk. Tambahkan produk pertama!
                        </td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>

{{-- ===================== TAB KATEGORI ===================== --}}
<div id="tab-kategori" class="hidden">
    <div class="mb-6">
        <a href="{{ route('kategori.create') }}"
            class="inline-flex items-center justify-center gap-2 bg-[#c5cb9f] text-[#5a4a2f] font-medium px-5 py-2.5 rounded-xl text-sm hover:bg-[#b5bb8f] transition w-full sm:w-auto">
            <span class="material-icons-outlined !text-[18px]">add</span>
            Tambah Kategori
        </a>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        @php $emojis = ['🍲','🥤','🍟','🍜','🍛','🧆','🥗','🍱']; @endphp

        @forelse($kategoris as $index => $kategori)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
            
            {{-- Emoji + Info --}}
            <div class="flex items-start gap-4 p-5 flex-grow">
                <div class="text-4xl mt-1 flex-shrink-0">{{ $emojis[$index % count($emojis)] }}</div>
                <div class="min-w-0">
                    <h3 class="font-semibold text-gray-800 text-lg truncate">{{ $kategori->nama_kategori }}</h3>
                    <p class="text-xs text-gray-400 mt-1 break-words">{{ $kategori->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                </div>
            </div>

            {{-- Tombol Aksi Kategori di Bawah --}}
            <div class="flex items-center gap-2 px-4 py-3 bg-gray-50 border-t border-gray-100 mt-auto">
                <a href="{{ route('kategori.edit', $kategori->id_kategori) }}"
                    class="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg py-2 hover:bg-gray-100 transition">
                    <span class="material-icons-outlined !text-[16px]">edit</span>
                    Edit
                </a>
                <form action="{{ route('kategori.destroy', $kategori->id_kategori) }}" method="POST"
                    onsubmit="return confirm('Hapus kategori ini?')" class="flex-1 flex">
                    @csrf @method('DELETE')
                    <button type="submit"
                        class="w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-red-500 bg-white border border-red-100 rounded-lg py-2 hover:bg-red-50 transition cursor-pointer">
                        <span class="material-icons-outlined !text-[16px]">delete</span>
                        Hapus
                    </button>
                </form>
            </div>

        </div>
        @empty
        <div class="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4 text-center text-gray-400 py-12">
            Belum ada kategori. Tambahkan kategori pertama!
        </div>
        @endforelse
    </div>
</div>
@endsection

@push('scripts')
<script>
    function switchTab(tab) {
        document.getElementById('tab-produk').classList.add('hidden');
        document.getElementById('tab-kategori').classList.add('hidden');
        document.getElementById('tab-' + tab).classList.remove('hidden');

        const btnProduk   = document.getElementById('btn-produk');
        const btnKategori = document.getElementById('btn-kategori');
        const activeClass   = 'flex-1 sm:flex-none flex items-center justify-center gap-1 font-medium px-4 py-2 rounded-xl text-sm transition bg-[#c5cb9f] text-[#5a4a2f]';
        const inactiveClass = 'flex-1 sm:flex-none flex items-center justify-center gap-1 font-medium px-4 py-2 rounded-xl text-sm transition border border-gray-300 text-gray-700 hover:bg-gray-50';

        if (tab === 'produk') {
            btnProduk.className   = activeClass;
            btnKategori.className = inactiveClass;
        } else {
            btnKategori.className = activeClass;
            btnProduk.className   = inactiveClass;
        }
    }

    const activeTab = new URLSearchParams(window.location.search).get('tab') || 'produk';
    switchTab(activeTab);

    function toggleDropdown(id) {
        document.querySelectorAll('[id^="filter-"]').forEach(function(el) {
            if (el.id !== id) el.classList.add('hidden');
        });
        document.getElementById(id).classList.toggle('hidden');
    }

    document.addEventListener('click', function(e) {
        if (!e.target.closest('.relative')) {
            document.querySelectorAll('[id^="filter-"]').forEach(function(el) {
                el.classList.add('hidden');
            });
        }
    });

    function toggleStatus(btn, id) {
        btn.disabled = true;
        btn.style.opacity = '0.5';

        fetch(`/produk/${id}/toggle-status`, {
            method: 'PATCH',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json',
            }
        })
        .then(res => res.json())
        .then(data => {
            const status = data.status;
            btn.dataset.status = status;
            btn.textContent = status === 'tersedia' ? 'Tersedia' : 'Habis';
            btn.className = 'text-xs font-semibold px-3 py-1 rounded-full border transition-all cursor-pointer focus:outline-none ' +
                (status === 'tersedia'
                    ? 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100'
                    : 'bg-red-50 text-red-500 border-red-200 hover:bg-red-100');
        })
        .catch(() => alert('Gagal update status, coba lagi.'))
        .finally(() => {
            btn.disabled = false;
            btn.style.opacity = '1';
        });
    }
</script>
@endpush