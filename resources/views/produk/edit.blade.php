...
<div class="max-w-4xl mx-auto w-full">
    {{-- Header: Ukuran teks adaptif & rute kembali disesuaikan ke tab produk --}}
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl md:text-2xl font-bold tracking-tight text-gray-900">Edit Produk</h2>
    </div>
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-2xl p-6 text-center hover:border-[#c5cb9f] transition cursor-pointer bg-gray-50/50"
                    ...
                    <div id="preview-container-form" class="{{ $produk->gambar ? '' : 'hidden' }} mb-3">
                        <img id="preview-img-form" src="{{ $produk->gambar ? asset('storage/' . $produk->gambar) : '' }}" alt="Preview"
                            class="w-24 h-24 object-cover rounded-xl mx-auto shadow-sm">
                    </div>
                    <span class="material-icons-outlined text-gray-300 !text-[36px] {{ $produk->gambar ? 'hidden' : '' }}" id="upload-icon">add_photo_alternate</span>
                    <p class="text-sm font-semibold text-gray-500 mt-1" id="upload-text">
        ...
        <div class="w-full lg:w-72 static lg:sticky lg:top-6 order-1 lg:order-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5">
                ...
                <div id="preview-img-wrapper"
                    ...
                    <span class="material-icons-outlined text-gray-300 !text-[44px] {{ $produk->gambar ? 'hidden' : '' }}" id="preview-placeholder">fastfood</span>
                    <img id="preview-img-card" src="{{ $produk->gambar ? asset('storage/' . $produk->gambar) : '' }}" alt="" class="{{ $produk->gambar ? '' : 'hidden' }} w-full h-full object-cover rounded-xl">
                </div>

                {{-- Informasi Menu Live --}}
                <h3 id="preview-nama" class="font-bold text-gray-950 text-base truncate">{{ $produk->nama_produk }}</h3>
                <p id="preview-kategori" class="text-xs font-semibold text-gray-400 mt-0.5 mb-3">{{ $produk->kategori->nama_kategori ?? 'Kategori' }}</p>

                <div class="flex items-center justify-between gap-2 border-t border-gray-50 pt-3">
                    <p id="preview-harga" class="text-[#5a4a2f] font-extrabold text-sm">Rp {{ number_format($produk->harga, 0, ',', '.') }}</p>
                    <span id="preview-status"
                        class="text-[11px] font-bold px-3 py-1 rounded-full border shadow-sm whitespace-nowrap {{ $produk->status === 'tersedia' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-500 border-red-200' }}">
...
<script>
    ...
    // Fungsi update status khusus preview edit card
